...
for server in "${serverList[@]}"; do
  ...
  if [[ "${redisFPC}" == "${redisId}" ]]; then
    ...
    if [[ -n "${database}" ]]; then
      echo "--- Updating Redis full page cache database on server: ${database} ---"
      ini-set "${currentPath}/../env.properties" yes "${redisId}" database "${database}"
    fi
    if [[ -n "${cachePrefix}" ]]; then
      echo "--- Updating Redis full page cache prefix on server: ${database} ---"
      ini-set "${currentPath}/../env.properties" yes "${redisId}" prefix "${cachePrefix}"
    fi
    if [[ -n "${className}" ]]; then
      echo "--- Updating Redis full page cache class name on server: ${database} ---"
